...
export default function Amenities() {
  return (
    <section id="amenities" className="relative pb-20 lg:pb-28 pt-0 bg-brand-cream overflow-hidden select-none">
      ...
      <div className="max-w-7xl mx-auto space-y-12 relative z-10 px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center space-y-3">
          ...
          <div className="inline-flex items-center gap-2 bg-brand-gold/10 border border-brand-gold/20 px-3 py-1 rounded-none">
            ...
            <span className="text-[10px] font-bold tracking-[0.2em] text-brand-gold uppercase">Bespoke Inclusions</span>
          </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
          {amenitiesList.map((item) => (
            <div
              key={item.name}
              className="group relative w-full aspect-square rounded-2xl overflow-hidden bg-brand-dark border border-neutral-200/60 shadow-md hover:shadow-2xl transition-all duration-500 ease-out"
            >
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-brand-dark/95 via-brand-dark/40 to-transparent z-10 transition-opacity duration-300" />

              {/* Polished Gold Top Structural Border Accent - Visible on Hover */}
              <span className="absolute top-0 left-0 right-0 h-[2.5px] bg-brand-gold scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-center z-20" />
              ...
              <div className="absolute inset-x-0 bottom-0 p-4 sm:p-5 z-20 transform translate-y-1 sm:translate-y-2 group-hover:translate-y-0 transition-transform duration-500 ease-out flex flex-col space-y-1 whitespace-normal">
                ...
                {/* Clean inline geometric gold dot indicator and text */}
                <div className="flex items-center gap-2">
                  <span className="w-1 h-1 rounded-full bg-brand-gold shrink-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 hidden sm:inline-block" />
                  <h3 className="font-medium text-xs sm:text-sm lg:text-base text-white tracking-wide leading-tight drop-shadow-md">
                    {item.name}
                  </h3>
                </div>

                {/* Subtext description fading in neatly on hover */}
