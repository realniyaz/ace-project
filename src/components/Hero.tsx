...
export default function Hero() {
  ...
  return (
    <section id="hero" className="relative w-full min-h-screen bg-brand-cream lg:bg-brand-dark flex flex-col lg:flex-row lg:items-center overflow-hidden pt-16 lg:pt-0">
      ...
      <div className="relative w-full h-[40vh] sm:h-[45vh] lg:absolute lg:inset-0 lg:h-full z-0 shrink-0">
        ...
        {/* Tailored dark-gradient masks targeting elegant blend dynamics */}
        <div className="hidden lg:block absolute inset-0 bg-linear-to-r from-brand-dark/90 via-brand-dark/50 to-brand-dark/80 z-20" />
        <div className="hidden lg:block absolute inset-0 bg-radial-gradient from-transparent via-brand-dark/30 to-brand-dark/90 z-20" />
      </div>
      ...
      <div className="relative z-30 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8 lg:py-0 flex items-center">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-8 items-center w-full">
          ...
          <div className="lg:col-span-7 flex flex-col items-center lg:items-start text-center lg:text-left space-y-5 lg:space-y-6 text-brand-dark lg:text-white animate-fade-in-up">
            <div className="space-y-3.5 sm:space-y-4">
              {/* Gold status tag ribbon framework */}
              <div className="inline-flex items-center gap-2 bg-brand-gold/10 lg:bg-brand-gold/15 border border-brand-gold/30 lg:border-brand-gold/40 px-3 py-1 rounded-none backdrop-blur-md">
                <span className="w-1.5 h-1.5 rounded-full bg-brand-gold animate-pulse" />
                <span className="text-[10px] font-bold tracking-[0.2em] text-brand-gold lg:text-brand-gold-light uppercase">{projectData.status}</span>
              </div>
              
              <div className="space-y-1.5 sm:space-y-2">
                <span className="text-xs uppercase tracking-[0.2em] text-neutral-500 lg:text-brand-gold-light font-semibold block">
                  {projectData.location}
                </span>
                <h1 className="text-3xl sm:text-4xl xl:text-6xl font-light tracking-tight text-neutral-950 lg:text-white leading-[1.15] lg:leading-[1.1] max-w-2xl font-serif">
                  {projectData.title}
                </h1>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4 sm:gap-6 py-4 lg:py-5 border-y border-neutral-200 lg:border-white/10 w-full max-w-xl text-left">
              <div className="space-y-1">
                <span className="text-[9px] sm:text-[10px] text-neutral-400 lg:text-neutral-400 uppercase tracking-wider font-medium block">Land Parcel</span>
                <span className="text-xs sm:text-sm font-light text-neutral-700 lg:text-neutral-100">{projectData.landParcel}</span>
              </div>
              <div className="space-y-1">
                <span className="text-[9px] sm:text-[10px] text-neutral-400 lg:text-neutral-400 uppercase tracking-wider font-medium block">Total Suites</span>
                <span className="text-xs sm:text-sm font-light text-neutral-700 lg:text-neutral-100">{projectData.totalUnits} Units</span>
              </div>
              <div className="space-y-1">
                <span className="text-[9px] sm:text-[10px] text-neutral-400 lg:text-neutral-400 uppercase tracking-wider font-medium block">Elevation</span>
            ...
            <div className="pt-1 flex flex-col sm:flex-row items-center gap-3 sm:gap-4 justify-center lg:justify-start w-full">
              <span className="px-3 py-1.5 border border-brand-gold/20 lg:border-brand-gold/30 bg-white lg:bg-brand-dark/50 backdrop-blur-md text-[9px] sm:text-[10px] font-bold tracking-widest text-brand-gold uppercase shadow-xs">
                Ultra-Luxury 3 & 4 BHK Apartments
              </span>
              <div className="text-sm sm:text-base font-light text-neutral-500 lg:text-neutral-200">
                Starting Price: <span className="text-neutral-950 lg:text-white font-medium tracking-wide text-base sm:text-lg">{projectData.startingPrice}</span>
              </div>
            </div>
          </div>
